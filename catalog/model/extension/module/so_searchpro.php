...
class ModelExtensionModuleSosearchpro extends Model {
	...
	public function getProducts_iSell($data = array()) {
            ...
            $sql_prepare="DROP  TABLE IF EXISTS tmp_founded_products";#TEMPORARY TEMPORARY
            $sql_create ="CREATE  TABLE tmp_founded_products (`product_id` INT NULL,`sort_priority` INT NULL)";
            $this->db->query($sql_prepare);
            ...
            $sql_code_match="INSERT INTO tmp_founded_products SELECT product_id,15 FROM ".DB_PREFIX."product p WHERE $where";#
            ...
            $condition_and[]=implode(' OR ',$condition_or);
            
            
            
            $where.="(".implode(") AND (",$condition_and).")";

            die($sql_code_match.$where);
            ...
            if (!empty($data['filter_category_id'])) {
                    if (!empty($data['filter_sub_category'])) {
                            $sql_table .= " FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (cp.category_id = p2c.category_id)";
                    } else {
                            $sql_table .= " FROM " . DB_PREFIX . "product_to_category p2c";
                    }

                    if (!empty($data['filter_filter'])) {
                            $sql_table .= " LEFT JOIN " . DB_PREFIX . "product_filter pf ON (p2c.product_id = pf.product_id) LEFT JOIN " . DB_PREFIX . "product p ON (pf.product_id = p.product_id)";
                    } else {
                            $sql_table .= " LEFT JOIN " . DB_PREFIX . "product p ON (p2c.product_id = p.product_id)";
                    }
            } else {
                    $sql_table .= " FROM " . DB_PREFIX . "product p";
            }
