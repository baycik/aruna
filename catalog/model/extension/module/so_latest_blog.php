...
class ModelExtensionModuleSolatestblog extends Model {
	...
	public function getListBlogs( $data ){
		...
		{
			$sql = ' SELECT b.*,bd.article_title,bd.description, (SELECT COUNT(*) FROM '.DB_PREFIX.'simple_blog_comment bc WHERE bc.simple_blog_article_id = b.simple_blog_article_id) AS comment,(SELECT bv.view FROM '.DB_PREFIX.'simple_blog_view bv WHERE bv.simple_blog_article_id = b.simple_blog_article_id) AS view  FROM '.DB_PREFIX.'simple_blog_article b LEFT JOIN '.DB_PREFIX.'simple_blog_article_description bd ON b.simple_blog_article_id=bd.simple_blog_article_id  and bd.language_id='.(int)$this->config->get('config_language_id').' ' ;
	...
	public function getUsers(){
		$sql = "SELECT * FROM `" . DB_PREFIX . "simple_blog_author`";
        $query = $this->db->query( $sql );
        $users = $query->rows;
        $output = array();
        foreach( $users as $user ){
            $output[$user['simple_blog_author_id']] = $user['name'];
        }
        return $output;
    }
