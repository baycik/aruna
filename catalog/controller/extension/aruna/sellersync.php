	...
	public function index(){
        ...
        public function importUserData (){
            ...
            foreach($import_array as $item){
                $this->model_extension_aruna_import->importCategories(json_decode(json_encode($item)), $seller_id);
            }
        }
        ...
        public function parse2 (){
            if (($handle = fopen("https://happywear.ru/exchange/xml/price-list.csv", "r")) !== FALSE) {
                ...
                $this->load->model('extension/aruna/sellersync');
                $i=0;
                while (($data = fgetcsv($handle, 5000, ";")) !== FALSE && $i++<1000) {
                    ...
                    if( $row['model'] ){
                        $this->model_extension_aruna_sellersync->insert_parsed_row($sync_id, $row); 
                    }
