...
class ControllerExtensionModuleISSWebapp extends controller {
    ...
    private function installWebappHeadTags( $webapp_data ){
        if( !$webapp_data['iss_webapp_status'] ){
            ...
            $code = 'iSellSoft-WebApp-DynamicMod';
        ...
        $result = $this->db->query("SELECT extension_install_id FROM `" . DB_PREFIX . "extension_path` WHERE path LIKE '%/iss_webapp.php'");
        ...
        $xml=<<<EOT
<modification>
    <name>iSellSoft WebApp DynamicMod</name>
    <code>iSellSoft-WebApp-DynamicMod</code>
    <version>1.0</version>
