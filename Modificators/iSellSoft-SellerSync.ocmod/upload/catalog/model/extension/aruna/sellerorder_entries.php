...
class ModelExtensionArunaSellerOrderEntries extends Model {
        ...
        public function updateSellerOrderProduct ($data){
            ...
            $this->db->query("
                UPDATE " . DB_PREFIX . "purpletree_order_total 
                SET 
                    value = (SELECT 
                            SUM(total) AS order_total
                        FROM
            ...
            $this->db->query("UPDATE " . DB_PREFIX . "purpletree_order_total SET value = '". $total ."' WHERE order_id = '".$data['order_id']."' AND code = 'total'");
        ...
        public function deleteSellerOrderProduct ($data){
            ...
            if((int)$order_products->row['total'] == 0){
                ...
                $this->db->query("DELETE FROM " . DB_PREFIX . "purpletree_order_total WHERE order_id = '".$data['order_id']."'");
        ...
        public function addSellerOrderProduct ($data){
            ...
            $this->db->query("INSERT INTO " . DB_PREFIX . "purpletree_vendor_orders 
                                SET
                                ...
                                order_status_id = (SELECT distinct pvo.order_status_id FROM oc_purpletree_vendor_orders pvo WHERE pvo.order_id = '".$data['order_id']."'), 
                                created_at =  NOW(),
                                updated_at =  NOW()
                                ");
        ...
        public function getOrderIds ($seller_id){
            $sql="
                ...
                WHERE 
                    (SELECT DISTINCT pvo.seller_id FROM oc_purpletree_vendor_orders pvo WHERE op.order_id = pvo.order_id LIMIT 1) = '".$seller_id."'
                ";
            $query = $this->db->query($sql);
            return $query->rows;
        }
		...
		$sql = "SELECT DISTINCT
                ...
                $sql .= "WHERE ";
                  ...
                  if (isset($data['filter_seller_id']) && $data['filter_seller_id'] != '*' ){
                        ...
                        $sql .= " (SELECT DISTINCT pvo.seller_id FROM oc_purpletree_vendor_orders pvo WHERE op.order_id = pvo.order_id LIMIT 1) = '".$seller_id."' ";
                } else {
                        $sql .= " ";
                }
                ...
                if (isset($data['filter_sync_id']) && $data['filter_sync_id'] != '*' ){
                        ...
                        $sql .= " AND bsl.sync_id  = '".(int)$sync_id . "' ";
                } else {
                        $sql .= "  ";
                }
